...
from monitor.discovery import _workspace_scan_roots, enrich_discovery_with_workspaces
...
class WorkspaceDiscoveryTests(unittest.TestCase):
    ...
    def test_workspace_scan_roots_do_not_promote_home_or_user_parent(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            home = Path(raw)
            workspace = home / "Desktop" / "ClientApp"
            workspace.mkdir(parents=True)

            roots = _workspace_scan_roots([workspace], home=home)

        self.assertIn(workspace.resolve(), roots)
        self.assertNotIn(workspace.parent.resolve(), roots)
        self.assertNotIn(home.resolve(), roots)
        self.assertNotIn(home.parent.resolve(), roots)
