...
def _cursor_note(snapshot: dict[str, Any]) -> str:
    ...
    if not cursor.get("available"):
        return ""
    return "Cursor local records expose prompt-side token totals, but no model/provider billing ledger was found."
...
def _grok_rows(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    """Extract Grok Build usage rows from the collector output for billing estimates."""
    grok = snapshot.get("grok") or {}
    usage = (grok.get("database") or {}).get("usage") or {}
    ...
    if by_model:
        ...
        for m in by_model:
            ...
            tokens = {
                "input": _to_int(m.get("inputTokens")),
                "output": _to_int(m.get("outputTokens")),
                "total": _to_int(m.get("totalTokens")),
            }
            if tokens["total"] > 0:
                rows.append(
                    {
                        ...
                        "model": model,
                        "tokens": tokens,
                        "threads": _to_int(m.get("events")),
                        "costMicros": _to_int(m.get("costMicros")),
                    }
                )
        return rows

    # Fallback to top-level summary if byModel is empty
    summary = grok.get("summary") or {}
    total = _to_int(summary.get("totalTokens") or summary.get("tokens"))
    if total > 0:
        return [
            {
                "source": "grok",
