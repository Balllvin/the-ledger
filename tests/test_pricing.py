...
class PricingTests(unittest.TestCase):
    ...
    def test_cursor_prompt_tokens_without_model_ledger_are_marked_unavailable_for_billing(self) -> None:
        snapshot = {"cursor": {"available": True, "summary": {"tokens": 1500, "generations": 3}}}

        billing = build_billing_estimate(snapshot)

        self.assertFalse(billing["sources"]["cursor"]["available"])
        self.assertIn("no model/provider billing ledger", billing["sources"]["cursor"]["note"])
        self.assertEqual(billing["sources"]["cursor"]["totalUsd"], 0)

    def test_cursor_rows_are_not_priced_even_if_activity_shape_mentions_models(self) -> None:
        snapshot = {
            "cursor": {
                "available": True,
                "summary": {"generations": 3},
                "billing": {"byModel": [{"model": "gpt-5", "input": 1_000_000, "output": 1_000_000, "total": 2_000_000}]},
            }
        }
        ...
        self.assertFalse(billing["sources"]["cursor"]["available"])
        self.assertEqual(billing["sources"]["cursor"]["models"], [])
        self.assertEqual(billing["sources"]["cursor"]["pricedTokens"], 0)
        self.assertEqual(billing["sources"]["cursor"]["totalUsd"], 0)
