...
def cursor_roots(home: Path | None = None) -> dict[str, Path]:
    ...
    return {
        ...
        "aiTracking": base / ".cursor" / "ai-tracking",
    }
...
def collect_cursor(home: Path | None = None) -> dict[str, Any]:
    ...
    ai_tracking = collect_ai_code_tracking(roots["aiTracking"] / "ai-code-tracking.db")
    ...
    return sanitize(
        {
            ...
            "aiTracking": ai_tracking,
            "summary": _cursor_summary(global_state, workspaces, logs, process_monitor, ai_tracking),
        }
    )
...
def collect_global_state(db: Path) -> dict[str, Any]:
    ...
    try:
        ...
        result["tokenUsage"] = _cursor_disk_kv_token_usage(con)
    except sqlite3.Error as exc:
        result["error"] = safe_preview(exc)
    finally:
        con.close()
...
def collect_ai_code_tracking(db_path: Path) -> dict[str, Any]:
    result: dict[str, Any] = {
        ...
        "scoredCommits": 0,
        "aiCommitPercentages": [],
        "composerLinesFromCommits": 0,
        "tabLinesFromCommits": 0,
        "trackingStart": None,
    }
    ...
    try:
        result["available"] = True
        # tracking_state
        row = con.execute("SELECT value FROM tracking_state WHERE key = 'trackingStartTime'").fetchone()
        if row:
            try:
                ts = json.loads(row[0]).get("timestamp")
                if ts:
                    result["trackingStart"] = timestamp_to_iso(ts / 1000 if ts > 1e12 else ts)
            except Exception:
                pass

        # ai_code_hashes - the real per-event trace (678 events on this laptop)
        events = 0
        by_source: Counter[str] = Counter()
        by_model: Counter[str] = Counter()
        by_day: Counter[str] = Counter()
        for row in con.execute("SELECT source, model, createdAt FROM ai_code_hashes"):
            events += 1
            src = str(row[0] or "unknown")
            mdl = str(row[1] or "unknown")
            day = ""
            try:
                ts = int(row[2] or 0)
                if ts:
                    from datetime import datetime, timezone
                    day = datetime.fromtimestamp(ts / 1000 if ts > 1e12 else ts, tz=timezone.utc).date().isoformat()
            except Exception:
                pass
            by_source[src] += 1
            by_model[mdl] += 1
        ...
        result["eventsBySource"] = dict(by_source.most_common())
        result["eventsByModel"] = dict(by_model.most_common())
        result["eventsByDay"] = dict(sorted(by_day.items(), reverse=True))
        # expose as tokens-shaped for the shared multi-system timeline (proxy units = events, not OpenAI tokens)
        result["timeline"] = [{"day": d, "tokens": c} for d, c in sorted(by_day.items())]

        # scored_commits - commit level AI attribution (9 commits at 95-100% AI)
        commits = 0
        ai_pcts: list[float] = []
        comp_lines = 0
        tab_lines = 0
        for row in con.execute("SELECT v2AiPercentage, composerLinesAdded, tabLinesAdded FROM scored_commits"):
            commits += 1
            try:
                p = float(row[0]) if row[0] else 0.0
                ai_pcts.append(p)
            except Exception:
                pass
            comp_lines += int(row[1] or 0)
            tab_lines += int(row[2] or 0)
        result["scoredCommits"] = commits
        result["aiCommitPercentages"] = [round(p, 2) for p in ai_pcts]
        result["composerLinesFromCommits"] = comp_lines
        result["tabLinesFromCommits"] = tab_lines
    except sqlite3.Error as exc:
        result["error"] = safe_preview(exc)
    finally:
        con.close()
...
def _cursor_disk_kv_token_usage(con: sqlite3.Connection) -> dict[str, Any]:
    result = {
        "available": False,
        "records": 0,
        "recordsWithTokens": 0,
    ...
    try:
        rows = con.execute("select value from cursorDiskKV where key like 'composerData:%'")
    ...
    for row in rows:
        result["records"] += 1
        data = _json_dict(row["value"] if isinstance(row, sqlite3.Row) else row[0])
        tokens = _composer_context_tokens(data)
        if tokens <= 0:
            continue
        result["recordsWithTokens"] += 1
        result["tokens"]["total"] += tokens
        result["tokens"]["input"] += tokens
        day = _composer_day(data)
        if day:
            by_day[day] += tokens

    result["days"] = [{"day": day, "tokens": int(tokens)} for day, tokens in sorted(by_day.items())]
    return result


def _composer_context_tokens(data: dict[str, Any]) -> int:
    prompt_breakdown = data.get("promptTokenBreakdown") if isinstance(data.get("promptTokenBreakdown"), dict) else {}
    for value in (
        prompt_breakdown.get("totalUsedTokens"),
...
def _composer_day(data: dict[str, Any]) -> str | None:
...
def _cursor_summary(global_state: dict[str, Any], workspaces: dict[str, Any], logs: dict[str, Any], process_monitor: dict[str, Any], ai_tracking: dict[str, Any] | None = None) -> dict[str, Any]:
    workspace_rows = workspaces.get("workspaces") or []
    daily_totals = ((global_state.get("dailyStats") or {}).get("totals") or {})
    token_usage = global_state.get("tokenUsage") or {}
    tokens = token_usage.get("tokens") or {}
    ai = ai_tracking or {}
    return {
        ...
        "tokenRecords": int(token_usage.get("recordsWithTokens") or 0),
        "aiEvents": int(ai.get("events") or 0),
        "aiScoredCommits": int(ai.get("scoredCommits") or 0),
        "aiComposerLines": int(ai.get("composerLinesFromCommits") or 0),
        "aiTabLines": int(ai.get("tabLinesFromCommits") or 0),
        "aiCommitPercentages": ai.get("aiCommitPercentages") or [],
        "aiTrackingStart": ai.get("trackingStart"),
        "timeline": token_usage.get("days") or [],
    }
